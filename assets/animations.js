...
const SCROLL_ANIMATION_TRIGGER_CLASSNAME = 'sf-scroll-trigger';
const SCROLL_ANIMATION_OFFSCREEN_CLASSNAME = 'sf-scroll-trigger--offscreen';
const SCROLL_ANIMATION_CANCEL_CLASSNAME = 'sf-scroll-trigger--cancel';
function onIntersection(elements, observer) {
  elements.forEach((element, index) => {
    if (element.isIntersecting) {
      ...
      if (element.target.classList.contains('sf__sidebar')) {
        if (MinimogTheme.config.mqlMobile) {
          element.target.classList.remove('sf-scroll-trigger');
        }
...
function initializeScrollAnimationTrigger() {
  ...
  let isDesignModeEvent = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  const animationTriggerElements = Array.from(rootEl.getElementsByClassName(SCROLL_ANIMATION_TRIGGER_CLASSNAME));
  if (animationTriggerElements.length === 0) return;
  if (isDesignModeEvent) {
    animationTriggerElements.forEach(element => {
      // element.classList.add('sf-scroll-trigger--design-mode');
    });
    return;
  }
